...
<body>
	<header>
		...
		<ul>
			...
			<li><a href="admin-vacatures.php">Vacaturen</a></li>
			<li><a href="admin-sollicitanten-vacature.php">Sollicitanten per vacature</a></li>
			<li><a href="logout.php">Log uit</a></li>
		</ul>
	</header>
	<div class="inleiding center">
		<p>Op de bedrijfspagina kunt u als admin CRUD toepassen. Klik op CREATE om nieuwe bedrijven aan te melden, klik op READ om een tabel te bekijken met alle aangemelden bedrijven, klik UPDATE om gegevens aan te passen van bedrijven die zich hebben aangemeld en klik DELETE om bedrijven te verwijderen.</p>
	</div>
	<form name="formulier" action="" method="POST">
		<h2>Bekijk de bedrijventabel:</h2>
			<p>Klik op create om een bedrijf aan te melden, read voor een overzicht van bedrijven, update om bedrijfsgegevens aan te passen en delete om een bedrijf te verwijderen.</p>
		<div class="row">
			<div class="col-6">
		...
		<div class="row">
			...
			<?php 
				...
				if(isset($_POST["read"])) {
					echo "<h3>Zie hier alle bedrijven:</h3>";
					$query = "SELECT naam FROM users WHERE soort = 'bedrijf' ORDER BY naam ASC";
				...
				if(isset($_POST["aanmaken"]) ) {
					...
					$data = array("$_POST[naam]", "$_POST[email]", "$_POST[password]", "bedrijf");
					try {
						$insert->execute($data);
						echo "<script>alert('uw heeft $_POST[naam] als bedrijf toegevoegd.');</script>";
					}
					catch(PDOException $e) {
				...
				if(isset($_POST["delete"]) ) {
					...
					echo '<form name="formulier" method="POST">
					<h3>Bedrijf accounts verwijderen:</h3>
					<p>Kies een account:</p>
					<select name="select"><option></option>';
					foreach( $updateArray as $val){
				...
				if(isset($_POST["verwijderen"]) ) {
					...
					$query2 = "DELETE FROM vacatures WHERE bedrijf = '$select'";
					$query3 = "DELETE FROM sollicitatiehistory WHERE bedrijf = '$select'";
					$insert = $database->prepare($query);
